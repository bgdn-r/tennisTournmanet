...
class Prelims extends Tournament {
  ...
  #prelimination() {
    // 4)NOTE If there is only 2 winners get one winner and return it
    ...
    if (this.#prelimPlayers.length % 2 === 0) {
      ...
      // 4)NOTE If even number then recurse
      if (this.#prelimPlayers.length % 2 === 0) {
        // 5)NOTE Recurse the function
        this.#prelimination();
      } else {
        // 6)NOTE If odd number add byePasser and then recurse
